...
let FormMixin = Base => class extends Base {
    ...
    _setOriginalModels()
    {
        ...
        for (let i = 0; i < this.form_fields.length; i++) {
            ...
            this.originalModels[form_field.id] = this[`${form_field.id}Model`] || null
        }
    }
    ...
    _getModel(id) {
        ...
        if (this.translatable) {
            // set not existing object keys
            if(!(fallback_locale in this.attributes) || this.attributes[fallback_locale] === undefined) {
                this.attributes[fallback_locale] = {}
            }

            if (!(form_field.local_key in this.attributes[fallback_locale])) {
                this.attributes[fallback_locale][form_field.local_key] = null;
            }
        }
        ...
        if (form_field.translatable) {
            ...
            if(this.attributes[lng][form_field.local_key] === null && form_field.type == ('wysiwyg' || 'textarea')) {
