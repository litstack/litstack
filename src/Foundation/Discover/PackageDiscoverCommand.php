...
class PackageDiscoverCommand extends LaravelPackageDiscoverCommand
{
    ...
    public function __construct()
    {
        ...
        $this->manifestPath = base_path('bootstrap/cache/lit.php');
        parent::__construct();
    }
    ...
    public function handle(PackageManifest $manifest)
    {
        ...
        foreach (array_keys($this->packages) as $package) {
            $this->line("Discovered Lit Package: <info>{$package}</info>");
        }
    ...
    /**
     * Find Lit packages in vendor/composer/installed.json.
     *
     * @param string $vendorPath
     *
     * @return array
     */
    public function findLitPackages(string $vendorPath)
    {
        ...
        return $this->filterLitPackages($packages);
    }

    /**
     * Filter composer packages for lit packages.
     *
     * @return array
     */
    public function filterLitPackages(array $packages)
    {
        // Filter for packages containing "extra": {"lit": ...}.
        return collect($packages['packages'] ?? $packages)->mapWithKeys(function ($package) {
            return [$this->format($package['name']) => $package['extra']['lit'] ?? []];
        })->filter()->all();
    }
    ...
    {
        $this->packages = [];
        $this->packages = $this->findLitPackages($this->vendorPath);
